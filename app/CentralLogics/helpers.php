    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['add_ons'] = AddOn::query()->whereIn('id', json_decode($item['add_ons']) ?? [])->get();
                $item['allergies'] = Allergy::query()->whereIn('id', json_decode($item['allergies']) ?? [])->get();
            ...
            $data_allergies = $data['allergies'];
            $allergy_ids = [];
            if(gettype($data_allergies) != 'array') {
                $allergy_ids = json_decode($data_allergies);

            } elseif(gettype($data_allergies) == 'array' && isset($data_allergies[0]['id'])) {
                foreach($data_allergies as $allergy) {
                    $allergy_ids[] = $allergy['id'];
                }

            } else {
                $allergy_ids = $data_allergies;
            }
            ...
            $data['add_ons'] = AddOn::query()->whereIn('id', $addon_ids ?? [])->get();
            $data['allergies'] = Allergy::query()->whereIn('id', $allergy_ids ?? [])->get();
    ...
    public static function order_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['allergy_ids'] = json_decode($item['allergy_ids']);
                array_push($storage, $item);
            }
            $data = $storage;
        } else {
            $data['add_on_ids'] = json_decode($data['add_on_ids']);
            $data['allergy_ids'] = json_decode($data['allergy_ids']);
            ...
            foreach ($data->details as $detail) {
                ...
                $detail->product_details->allergies = gettype($detail->product_details->allergies) != 'array' ? json_decode($detail->product_details->allergies) : $detail->product_details->allergies;
                ...
                $detail->allergy_ids = gettype($detail->allergy_ids) != 'array' ? json_decode($detail->allergy_ids) : $detail->allergy_ids;
...
//                $data[] = ['id'=> $addon->id, 'name'=>$addon->name, 'price'=>$addon->price, 'quantity'=> $add_on_qty];
    ...
    public static function order_details_formatter($details)
    {
        if ($details->count() > 0) {
            foreach ($details as $detail) {
                ...
                $detail['allergy_ids'] = gettype($detail['allergy_ids']) != 'array' ? (array) json_decode($detail['allergy_ids'], true) : (array) $detail['allergy_ids'];
    ...
    public static function product_formatter($product)
    {
        ...
        $product['allergies'] = gettype($product['allergies']) != 'array' ? (array)json_decode($product['allergies'], true) : (array)$product['allergies'];
        ...
        try {
            ...
            $allergies = [];
            foreach ($product['allergies'] as $allergy_id) {
