                        ...
                        @if($con->image != null && $con->image != "null" && count(json_decode($con->image, true)) > 0)
                                ...
                                @foreach($image_array as $image)
                                    <a href="{{$image}}" data-lightbox="{{$con->id . $image}}" >
                                        <img class="rounded" src="{{$image}}" onerror="this.src='{{asset('public-assets/assets/admin/img/900x400/img1.jpg')}}'" />
                                    </a><br>
                               @endforeach
                                </div>
                                <span class="time_date">  {{\Carbon\Carbon::parse($con->created_at)->format('h:m a | M d')}}</span>
                                ...
                                @foreach($image_array as $key=>$image)
                                    ...
                                    <div class="col-12 @if(count(json_decode($con->image, true)) > 1) col-md-6 @endif">
                                        <a href="{{asset('storage/app/public/conversation').'/'.$image_url}}" data-lightbox="{{$con->id . $image_url }}" >
                                            <img class="rounded" src="{{asset('storage/app/public/conversation').'/'.$image_url}}" onerror="this.src='{{asset('public-assets/assets/admin/img/900x400/img1.jpg')}}'" />
                                        </a><br>
                                    </div>
                                @endforeach
                                </div>
...
<script>
    function spartanMultiImagePicker() {
        ...
        $("#coba").spartanMultiImagePicker({
            ...
            maxFileSize: '',
            {{--placeholderImage: {--}}
                {{--    image: '{{asset('public-assets/assets/back-end/img/400x400/img2.jpg')}}',--}}
                {{--    width: '100%',--}}
                {{--},--}}
