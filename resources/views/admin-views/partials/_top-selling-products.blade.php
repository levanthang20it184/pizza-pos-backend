            ...
            @if(isset($item->product))
                <a class="d-flex justify-content-between align-items-center text-dark" href='{{route('admin.product.view',[$item['product_id']])}}'">
                    <div class="media align-items-center gap-2">
                        <img class="rounded avatar avatar-lg" src="{{ asset('storage/product').'/'.$item->product->image  ?? '' }}" onerror="this.src='{{asset('public-assets/assets/admin/img/400x400/img2.jpg')}}'" alt="{{$item->product->name}} image">
